...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL('/connect?success=github', request.url))
  } catch (error) {
    console.error('GitHub OAuth error:', error)
    return NextResponse.redirect(new URL('/connect?error=github_auth_failed', request.url))
  }
} 